...
def generate_clicked(task: worker.AsyncTask):
    ...
    yield gr.update(visible=True, value=modules.html_change.make_progress_html(1, 'Waiting for task to start ...')), \
    ...
    while not finished:
        ...
        if len(task.yields) > 0:
            ...
            if flag == 'preview':
                ...
                yield gr.update(visible=True, value=modules.html_change.make_progress_html(percentage, title)), \
...
def get_username(request: gr.Request):
    return gr.HTML.update(value=f'<p id="username">{request.username}</p>')
...
with shared.gradio_root as demo:
    ...
    with gr.Row(elem_classes="header_row"):
        gr.HTML('<img class="logo_main" src="https://trova-images.s3.amazonaws.com/Copia+de+Mystica+Logo+Black+(1).png"/>')
        #input - label and output -label
        
        user_ip = gr.HTML(value="Not logged in", color="#000")
        demo.load(get_username, inputs=None, outputs=user_ip)
        
        #gr.LogoutButton(value="SignOut")

    with gr.Row(elem_classes="prompt_body"):
        with gr.Column(scale=2):
            with gr.Row(elem_classes="welcome_label_row"):
                with gr.Column(elem_classes="welcome_label_col"):
                    gr.HTML('<h3 class="title">Mystica AI 🚀</h3>')
                    gr.HTML('<p class="text">What do you want to create today?</p>')
            with gr.Row(elem_classes="preview_result_row"):
                progress_window = grh.Image(label='Preview', show_label=True, visible=False, height=768,
                                            elem_classes=['main_view'])
                progress_gallery = gr.Gallery(label='Finished Images', show_label=True, object_fit='contain',
                                              height=768, visible=False, elem_classes=['main_view', 'image_gallery'])
            progress_html = gr.HTML(value=modules.html_change.make_progress_html(32, 'Progress 32%'), visible=False,
                                    elem_id='progress-bar', elem_classes='progress-bar')
            gallery = gr.Gallery(label='Gallery', show_label=False, object_fit='contain', visible=True, height=768,
                                 elem_classes=['resizable_area', 'main_view', 'final_gallery', 'image_gallery'],
                                 elem_id='final_gallery')
            with gr.Row(elem_classes='type_row_prompt'):
                with gr.Column(scale=17):
                    prompt = gr.Textbox(show_label=False, placeholder="Type prompt here or paste parameters.", elem_id='positive_prompt',
                                        container=False, autofocus=True, elem_classes='type_row_prompt', lines=1024)

                    default_prompt = modules.config.default_prompt
                    if isinstance(default_prompt, str) and default_prompt != '':
                        shared.gradio_root.load(lambda: default_prompt, outputs=prompt)

            with gr.Row(elem_classes="assets_row",visible=True):
                            ip_images = []
                            ip_types = []
                            ip_stops = []
                            ip_weights = []
                            ...
                            for _ in range(flags.controlnet_image_count):
                                with gr.Column(visible=True):
                                    gr.HTML('<label class="label_bold">Selfie Here 👇</label>')
                                    ip_image = grh.Image(show_download_button=True,label='Drag your Selfie Here!', source='upload', type='numpy', height=467)
                            ...
                            with gr.Column():
                                gr.HTML('<label class="label_bold">Model Here 👇</label>')
                                inpaint_input_image = grh.Image(show_download_button=True,label='Drag your Model Image!', source='upload', type='numpy', tool='sketch', height=467, brush_color="#FFFFFF", elem_id='inpaint_canvas')
                                
                                inpaint_mode = gr.Dropdown(visible=False,choices=modules.flags.inpaint_options, value=modules.flags.inpaint_option_default, label='Method')
                                inpaint_additional_prompt = gr.Textbox(visible=False,placeholder="Describe what you want to inpaint.", elem_id='inpaint_additional_prompt', label='Inpaint Additional Prompt')
                                outpaint_selections = gr.CheckboxGroup(visible=False,choices=['Left', 'Right', 'Top', 'Bottom'], value=[], label='Outpaint Direction')
                            ...
                            with gr.Column(visible=True) as inpaint_mask_generation_col:
                                gr.HTML('<label class="label_bold">Mask Here 👇</label>')
                                inpaint_mask_image = grh.Image(show_download_button=True,label='Mask Upload!', source='upload', type='numpy', tool='sketch', height=415)
                                ...
                                #hacer parecido del .click para el label que se envia como input y output
                                def generate_mask(image, mask_model, cloth_category, sam_prompt_text, sam_model, sam_quant, box_threshold, text_threshold):
                                    from mask_generate_model.main import generate_mask_model
                                    return generate_mask_model(image)
            ...
            with gr.Row(elem_classes="actions_row",visible=True):
                with gr.Column(min_width=0):
                    generate_button = gr.Button(label="Generate", value="Generate 💡", elem_classes='type_row', elem_id='generate_button', visible=True)
                    open_modal = gr.Button(label="Send Feedback", value="Open Modal 💡", elem_classes='type_row', elem_id='open_modal', visible=False)
                    ...
                    open_modal.click(fn= openModal,_js="handleModal")
            ...
            with gr.Row(elem_classes='advanced_check_row'):
                input_image_checkbox = gr.Checkbox(label='Input Image', value=True, container=True, elem_classes='min_check', visible=False)
                advanced_checkbox = gr.Checkbox(label='Advanced', value=modules.config.default_advanced_checkbox, container=True, elem_classes='min_check', visible= False)
            ...
            with gr.Row(visible=False) as image_input_panel:        
                with gr.Tabs():
                    ...
                    with gr.TabItem(label='Describe', visible=False) as desc_tab:
                        with gr.Row():
                            with gr.Column():
        ...
        generate_button.click(lambda: (gr.update(visible=True, interactive=True), gr.update(visible=True, interactive=True), gr.update(visible=False, interactive=False), [], True),
            ...
            .then(fn=lambda:None,_js='handleModal')
